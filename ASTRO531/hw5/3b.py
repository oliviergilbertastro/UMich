...
logT = (table["logT"])
T = 10**logT * u.K
grad_R_theo = 3/(64*np.pi*cst.sigma_sb*cst.G) * P/T**4 * rmo * L/M
grad_R_theo.decompose()
grad_R = table["gradr"]
...
ax2 = plt.subplot(111)
ax2.plot(logT, grad_R, ls="-", lw=2, color="red", label=r"$\nabla_R$ MESA")
...
ax2.set_yscale("log")
plt.tight_layout()
plt.savefig(r"ASTRO531\hw5\plot3b.pdf")
plt.show()