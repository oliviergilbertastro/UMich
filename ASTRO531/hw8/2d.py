...
for i, mass in enumerate([r"$0.5M_\odot$",r"$1M_\odot$",r"$5M_\odot$"]):
    ...
    #hist = hist[:end_dict[mass]]
    ax.plot(hist["log_center_T"], hist["log_center_Rho"], color="black", ls="-", lw=3, alpha=0.7)
    ax.plot(rgb["log_center_T"], rgb["log_center_Rho"], color="orange", ls="solid", lw=5, alpha=1, label="RGB")
    ax.plot(early_agb["log_center_T"], early_agb["log_center_Rho"], color="red", ls="solid", lw=5, alpha=1, label="Early AGB")
    ax.plot(hb["log_center_T"], hb["log_center_Rho"], color="blue", ls="solid", lw=5, alpha=1, label="HB")
    ax.plot(main_seq["log_center_T"], main_seq["log_center_Rho"], color="green", ls="solid", lw=3, alpha=1, label="MS")
    ...
    ax.text(ax.get_xlim()[0]+0.2,4,"Non-relativistic\ndegeneracy",color="blue",weight="bold",fontsize=16)
plt.tight_layout()
plt.savefig("ASTRO531/hw8/figures/2d.pdf")
plt.show()
